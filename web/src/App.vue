      ...
      <div class="h-screen flex flex-col justify-center items-center">
        <div class="flex flex-col w-fit gap-2 w-full">
          ...
          <div class="w-fit px-3 py-1 text-white rounded-xl mx-4 lg:mx-0 flex flex-row gap-2 items-center justify-center cursor-pointer hover:bg-zinc-600" :class="{'bg-zinc-600': ongoingRequest, 'bg-black': !ongoingRequest}" @mouseover="showStopIcon = true" @mouseleave="showStopIcon = false">
            ...
            <StopIcon class="w-[16px]" v-else-if="ongoingRequest && showStopIcon" @click="cancelOngoingRequest()"/>
...
<script setup>
  ...
  import { ArrowUpIcon, StopIcon } from '@heroicons/vue/20/solid';
  ...
  async function handleRequest() {
    ...
    try {
        ...
        // Return response
        return response?.data?.sentence_sentiments ?? false;
    } catch (error) {
  ...
  function cancelOngoingRequest() {
    if (typeof cancelRequest === 'function') {
      cancelRequest('Request cancelled manually');
      alert('Request has been cancelled manually');
      ongoingRequest.value = false; // Re-enable button and reset the loading state
      document.getElementById('requestButton').disabled = false;
    }
  }

  // Process sentiment data and map it into an array of objects
  function processSentiment(sentenceSentiments) {
    return sentenceSentiments.map(([sentence, sentiment, probabilities]) => {
      ...
      return {
        sentence: sentence,
        sentiment: sentiment,
        probabilities: probablitiesObject,
...
<style scoped>
.sentence-wrap {
  word-wrap: break-word; /* Ensures long sentences wrap efficiently */
  max-width: 100%; /* Ensures text does not overflow its container */
}
</style>
